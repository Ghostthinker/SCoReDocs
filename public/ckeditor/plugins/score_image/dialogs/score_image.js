...
CKEDITOR.dialog.add('score_image', function (editor) {
    ...
    var regexGetSizeOrEmpty = /(^\s*(\d+)(px)?\s*$)|^$/i,
        ...
        helpers = CKEDITOR.plugins.score_image,
        ...
        // Functions inherited from score_image plugin.
        ...
        lockButton, resetButton, widthField, heightField, metaField,
    ...
    function onChangeSrc() {
        ...
        if (value !== widget.data.src) {
            ...
            preLoader(value, function (image, width, height) {
                ...
                widthField.setValue(editor.config.score_image_prefillDimensions === false ? 0 : width);

                // Fill height field with the height of the new image.
                heightField.setValue(editor.config.score_image_prefillDimensions === false ? 0 : height);
        ...
        if (srcChanged) {
            ...
            clearTimeout(srcChangeTimeoput);
            srcChangeTimeoput = setTimeout(function () {
                    $.ajax({
                        type: "GET",
                        url: "/rest/file/" + filename,
                        success: function (data) {
                            // set metadata
                            metaField.setValue(data.meta);
                            // set if empty
                            var caption = widget.parts.caption
                            if (!caption) {
                                var caption = data.caption;
                                widget.setData('captionLabel', caption);
    ...
    var srcBoxChildren = [
        {
            ...
            commit: function (widget) {
                // update meta and caption
                var filename = this.getValue().split("/").pop();
                $.ajax({
                    type: "PATCH",
                    url: "/rest/file/" + filename,
                    data: {'meta': metaField.getValue()},
                    success: function (data) {
        ...
        contents: [
            {
                ...
                elements: [
                    ...
                    },
                    {
                        ...
                        validate: editor.config.score_image_altRequired === true ? CKEDITOR.dialog.validate.notEmpty(lang.altMissing) : null
                    },
                    {
                        type: 'hbox',
                        widths: ['25%', '25%', '50%'],
                    ...
                    },
                    {
                        ...
                        setup: function (widget) {
                            var filename = widget.data.src.split("/").pop();;
                            // get metadata from db
                            var self = this;

                            // initial setup only
                            if(widget.data.metadata === undefined) {
                                $.ajax({
                                    type: "GET",
                                    url: "/rest/file/" + filename,
                                    success: function (data) {
                                        // set metadata
                                        self.setValue(data.meta);
                                    },
                                    error: function (e) {
                                        // todo
                                    },
                                ...
                                this.setValue(widget.data.metadata);
                            }
                        },
                        commit: function (widget) {
                            widget.setData('metadata', this.getValue());
                        }
                    },
                    {
                        id: 'hasCaption',
                        ...
                        setup: function (widget) {
                            ...
                            //override default checkbox to be checked
                            if (!widget.data.src) {
                                this.setValue(true);
                            }
                        },
