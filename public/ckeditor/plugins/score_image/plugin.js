...
(function () {
    ...
    CKEDITOR.plugins.add('score_image', {
        ...
        init: function (editor) {
            ...
            if (editor.plugins.detectConflict('score_image', ['easyimage'])) {
                return;
            }

            // Adapts configuration from original image plugin. Should be removed
            // when we'll rename score_image to image.
            var config = editor.config,
                lang = editor.lang.score_image,
                image = widgetDef(editor);

            // Since filebrowser plugin discovers config properties by dialog (plugin?)
            // names (sic!), this hack will be necessary as long as score_image is not named
            // Image. And since score_image will never be Image, for sure some filebrowser logic
            // got to be refined.
            config.filebrowserscore_imageBrowseUrl = config.filebrowserImageBrowseUrl;
            config.filebrowserscore_imageUploadUrl = config.filebrowserImageUploadUrl;
            ...
            CKEDITOR.dialog.add('score_image', this.path + 'dialogs/score_image.js');
        },
    ...
    // 		└──────┴───────────────────────────────┴─────────────────────────────┘
    //
    // Non-captioned widget (config.score_image_alignClasses defined)
    // 		┌──────┬───────────────────────────────┬─────────────────────────────┐
    // 		│Align │Internal form                  │Data                         │
    // 		├──────┼───────────────────────────────┼─────────────────────────────┤
    // 		│none  │<wrapper>                      │<img />                      │
    ...
    // 		└──────┴────────────────────────────────────────┴────────────────────────────────────────┘
    //
    // Captioned widget (config.score_image_alignClasses defined)
    // 		┌──────┬────────────────────────────────────────┬────────────────────────────────────────┐
    // 		│Align │Internal form                           │Data                                    │
    // 		├──────┼────────────────────────────────────────┼────────────────────────────────────────┤
    // 		│none  │<wrapper>                               │<figure />                              │
    ...
    function widgetDef(editor) {
        var alignClasses = editor.config.score_image_alignClasses,
            captionedClass = editor.config.score_image_captionedClass;
        ...
        return {
            ...
            init: function () {
                var helpers = CKEDITOR.plugins.score_image,
                ...
                if (editor.filter.checkFeature(this.features.dimension) && editor.config.score_image_disableResizer !== true) {
                    setupResizer(this);
                }
            ...
            // Overrides default method to handle internal mutability of score_image.
            ...
            getClasses: (function () {
                ...
                return function () {
                    ...
                    // Neither config.score_image_captionedClass nor config.score_image_alignClasses
    ...
    /**
     * A set of Enhanced Image (score_image) plugin helpers.
     *
     * @class
     * @singleton
     */
    CKEDITOR.plugins.score_image = {
        stateShifter: function (editor) {
            // Tag name used for centering non-captioned widgets.
            var doc = editor.document,
                alignClasses = editor.config.score_image_alignClasses,
                captionedClass = editor.config.score_image_captionedClass,
            ...
            var stateActions = {
                ...
                hasCaption: function (shift, oldValue, newValue) {
                    ...
                    if (newValue) {
                        var captionLabel = shift.widget.data.captionLabel ? shift.widget.data.captionLabel : editor.lang.score_image.captionPlaceholder;
                        // Create new <figure> from widget template.
                        var figure = CKEDITOR.dom.element.createFromHtml(templateBlock.output({
                            captionedClass: captionedClass,
                            captionPlaceholder: captionLabel
                        }), doc);
                ...
                link: function (shift, oldValue, newValue) {
                    if (shift.changed.link) {
                        ...
                        if (!newValue)
                            ...
                            var attributes = CKEDITOR.plugins.score_image.getLinkAttributesGetter()(editor, newValue);
    ...
    // <figure class="{config.score_image_captionedClass}"> elements.
    //
    ...
    function downcastWidgetElement(editor) {
        ...
        return function (el) {
            ...
            // inline styles or classes (score_image_alignClasses).
    ...
    function centerWrapperChecker(editor) {
        var captionedClass = editor.config.score_image_captionedClass,
            alignClasses = editor.config.score_image_alignClasses,
        ...
        return function (el) {
            ...
            // Centering wrapper got to be... centering. If score_image_alignClasses are defined,
    ...
    function setupResizer(widget) {
        ...
        resizer.setAttribute('title', editor.lang.score_image.resizer);
        ...
        resizer.on('mousedown', function (evt) {
            ...
            function getMaxSize() {
                var maxSize = editor.config.score_image_maxSize,
                    natural;
                ...
                natural = CKEDITOR.plugins.score_image.getNatural(image);
            ...
            if (dialog.name == 'link') {
                ...
                def.onShow = function () {
                    ...
                    if (widget && (widget.inline ? !widget.wrapper.getAscendant('a') : 1)) {
                        ...
                        // Hide the display text in case of linking score_image widget.
                        displayTextField.hide();
                    } else {
                        // Make sure that display text is visible, as it might be hidden by score_image integration
                        // before.
    ...
    // on configurations like config#score_image_alignClasses or
    // config#score_image_captionedClass.
    //
    // @param {CKEDITOR.editor}
    // @returns {Object}
    function getWidgetAllowedContent(editor) {
        var alignClasses = editor.config.score_image_alignClasses,
            rules = {
                ...
                figure: {
                    classes: '!' + editor.config.score_image_captionedClass
                },
                figcaption: true
            };
 ...
 *
 * Read more in the {@glink features/score_image documentation} and see the
 * {@glink examples/score_image example}.
 *
 *        // Changes the class to "captionedImage".
 *        config.score_image_captionedClass = 'captionedImage';
 *
 * @cfg {String} [score_image_captionedClass='image']
 * @member CKEDITOR.config
 */
CKEDITOR.config.score_image_captionedClass = 'image';
 ...
 * {@glink examples/score_image example}.
 *
 *        config.score_image_prefillDimensions = false;
 *
 * @since 4.5.0
 * @cfg {Boolean} [score_image_prefillDimensions=true]
 * @member CKEDITOR.config
 */
 ...
 * {@glink examples/score_image example}.
 *
 *        config.score_image_disableResizer = true;
 *
 * @since 4.5.0
 * @cfg {Boolean} [score_image_disableResizer=false]
 * @member CKEDITOR.config
 */
 ...
 *
 *        config.score_image_alignClasses = [ 'align-left', 'align-center', 'align-right' ];
 *
 ...
 *
 * @since 4.4.0
 * @cfg {String[]} [score_image_alignClasses=null]
 * @member CKEDITOR.config
 */
 ...
 *
 *        config.score_image_altRequired = true;
 *
 * Read more in the {@glink features/score_image documentation} and see the
 * {@glink examples/score_image example}.
 *
 * @since 4.6.0
 * @cfg {Boolean} [score_image_altRequired=false]
 * @member CKEDITOR.config
 */
 ...
 *
 * @since 4.12.0
 * @cfg {Object.<String, Number/String>} [score_image_maxSize]
 * @member CKEDITOR.config
 */
